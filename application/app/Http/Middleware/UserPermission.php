...
//        dd($request->user()->getAuthUserGeneralRole());
        if(Auth::check()){
            ...
            //Module
            $hasModule = $request->route()->action['module_name'] ?? false;
            if($hasModule){
                $moduleFilePath = 'module\\'.ucwords($hasModule).'\routes\routes.php';
                if(file_exists(base_path($moduleFilePath)) == true){
                    $request->attributes->add([
                        'activeModule' => $hasModule,
                        'module_name' => ucwords($hasModule),
                    ]);
                }
            }
